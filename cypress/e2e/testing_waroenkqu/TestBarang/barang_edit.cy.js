...
describe('Admin can see barang page', () => {
  it('Kategori test', () => {
   ...
   cy.get(':nth-child(1) > :nth-child(8) > form > .btn-primary').click({ force: true });

    cy.get('.card-header').contains('Edit Barang').and('be.visible');
    cy.get(':nth-child(3) > label').should('have.text','Kategori');
    cy.get(':nth-child(4) > label').should('have.text','nama_barang');
